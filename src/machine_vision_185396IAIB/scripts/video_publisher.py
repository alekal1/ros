    ...
    def publish_video_frame(self):
        while (self.cap.isOpened()):
            ...
            if self.frame_counter == self.video_frames:
                self.frame_counter = 0
                # Recaptures video when it's finished
                self.cap = cv2.VideoCapture(self.video[0])
            if ret == True:
