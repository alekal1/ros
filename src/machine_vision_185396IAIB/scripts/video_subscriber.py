...
class VideoSubscriber:
    ...
    def __init__(self):
        ...
        self.frame_publisher = rospy.Publisher('/processed_video', Image, queue_size=1000)
        self.bridge = CvBridge()
        self.min_value = np.array([20, 160, 160], dtype="uint8") # Minimum color range
        self.max_value = np.array([25, 255, 255], dtype="uint8") # Maximum color range
    ...
    def callback(self, video_frame_data):
        ...
        duck_mask = cv2.inRange(frame_hsv, self.min_value, self.max_value) # ??? 
